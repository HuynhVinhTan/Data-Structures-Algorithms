...
bool isPossible(int time, int K, int job[], int n)
{
    ...
    for (int i = 0; i < n;) {
        ...
        else { // Else add time of job to current time and
               // move to next job.
            curr_time += job[i];
            i++;
        }
...
// Returns minimum time required to finish given array of
// jobs k --> number of assignees T --> Time required by
// every assignee to finish 1 unit m --> Number of jobs
